...
function eventHandler() {
  ...
  document.addEventListener('click', function (event) {
    ...
    if (profileTarget && window.matchMedia('(max-width: 768px)').matches) {
      ...
      profileMenus.forEach((profileMenu) => {
  ...
  $('.show-more-js').on('click', function (event) {
    $(
      event.target.closest('.sPopularCategories').querySelector('.sPopularCategories__full-list'),
    ).addClass('active');
    $('body').addClass('fixed');
    event.preventDefault();
  });
  ...
  if (document.querySelector('.sProdCard__thumb-slider--thumb-js')) {
    ...
    sProdCardSwiper2 = new Swiper('.sProdCard__slider--js', {
      slidesPerView: 1,
      navigation: {
    ...
    sProdCardSwiper2.on('slideChange', () => {
      document.querySelectorAll('.sProdCard__slide iframe').forEach((item) => {
  ...
  if (sProdCardTabsSwiper) {
    const gridRows = Math.ceil(
      sProdCardTabsSwiper.querySelectorAll('.equipment-slider__slide').length / 4,
    );
  ...
  let scrollTop = document.querySelector('.all-pages-btn__btn--scrool-up-js');
  if (scrollTop) {
